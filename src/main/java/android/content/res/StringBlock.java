...
public class StringBlock {
  private final CharsetDecoder UTF16LE_DECODER;
  ...
  public String getHTML(int index) {
    ...
    while (true) {
      ...
      for (int k = 0; k != style.length; k += 3) {
        int end = style[k + 2];
        if (end != -1 && end < start) {
          if (offset <= end) {
  ...
  private StringBlock() {
    this.UTF16LE_DECODER = Charset.forName("UTF-16LE").newDecoder();
  ...
  private int[] getStyle(int index) {
    ...
    int count = 0;
    for (int k = offset; k < this.m_styles.length && this.m_styles[k] != -1; k++)
      count++; 
    if (count == 0 || count % 3 != 0)
      return null; 
    int[] style = new int[count];
    for (int i = offset, j = 0; i < this.m_styles.length && this.m_styles[i] != -1;)
